...
Site responsável por explicar o módulo:https://p5r.uk/quine-mccluskey/index.html
  ...
  [] Tentar realizar operações com números (e não com strings) ao descompactar o arquivo. Realizar operações sobre os minitermos numéricos da tabela verdade;
  [] work with numbers and not with strings (convert_miniterms_to_numbers, line 804; convert_numbers_to_miniterms, line 817; output_truth_table, line 1218);
  [] Colocar recursividade, colocar threads e passar funções para a GPU ao mesmo tempo em relação a algumas funções que demoram muito.
  ...
  def count_time_and_print (self):
    if self.show_time_left:
      ...
      if current_second != self.previous_second:
        ...
        self.time_left = (self.timestamp_differ_result) * (100 -current_percentage)
...
#      return reduce (lambda x, y: (ord (x) if type (x) == type (str()) else x)<<8|ord (y), string)
  ...
  def compact_file (cls, filename):
    ...
    if path.isfile (filename):
      ...
      if DEBUG and PRINT_LONG_TEXT and CLI_MODE:
        if ALLOW_PROMPT_MESSAGES:
          print(number)
