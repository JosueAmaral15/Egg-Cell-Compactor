...
Site responsável por explicar o módulo:https://p5r.uk/quine-mccluskey/index.html
  ...
  [] Resolver problema referente à linha 1016 com recursividade e inserção maior de zeros com deslocamento (colocar True para argumento da função na linha 1055 a fim de analisar recursividade);
  [] Colocar no algoritmo not cls.THREADS_AMMOUNT == 0 or cls.THREADS_AMMOUNT < cls.THREADS_MAX;
...
#      return reduce (lambda x, y: (ord (x) if type (x) == type (str()) else x)<<8|ord (y), string)
  ...
  def transform_content (cls, content, total_length):
    if total_length > 1 and (not cls.NUMBER_RECURSIVE_FUNCTIONS or cls.NUMBER_RECURSIVE_FUNCTIONS < cls.MAX_RECURSIVE) and virtual_memory()[1] > cls.MAX_FREE_MEMORY:
  ...
  def transform_content_revert (cls, content, total_length):
    if total_length > 8 and cls.NUMBER_RECURSIVE_FUNCTIONS < cls.MAX_RECURSIVE and virtual_memory()[1] > cls.MAX_FREE_MEMORY:
      cls.NUMBER_RECURSIVE_FUNCTIONS+=1
  ...
  def revert_big_number (cls, big_number, filename, is_binary, recursive = True):
    if ALLOW_PROMPT_MESSAGES and CLI_MODE:
      print(cls.MESSAGE_BIG_NUMBER_REVERT_FILE)
